...
-- Дата скрипта: 11.09.2012 15:52:31
...
--
-- Описание для таблицы lockreferencebook
--
CREATE TABLE IF NOT EXISTS lockreferencebook (
  id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT,
...
--
-- Описание для таблицы historyuseronwp
--
CREATE TABLE IF NOT EXISTS historyuseronwp (
  id INT(11) UNSIGNED NOT NULL AUTO_INCREMENT,
  CodUser INT(11) UNSIGNED NOT NULL,
  CodWorkerPlace INT(11) UNSIGNED NOT NULL,
  ...
  PRIMARY KEY (id),
  CONSTRAINT FK_historyuseronwp_users_CodUser FOREIGN KEY (CodUser)
    REFERENCES users(CodUser) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT FK_historyuseronwp_workerplace_CodWorkerPlace FOREIGN KEY (CodWorkerPlace)
    REFERENCES workerplace(CodWorkerPlace) ON DELETE CASCADE ON UPDATE CASCADE
)
ENGINE = INNODB
AUTO_INCREMENT = 32
...
CREATE TABLE IF NOT EXISTS historymoved (
  ...
  CONSTRAINT FK_historymoved_users_CodUser FOREIGN KEY (CodPerformer)
    REFERENCES users(CodUser) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT FK_historymoved_workerplace_CodWorkerPlace FOREIGN KEY (CodOldPlace)
    REFERENCES workerplace(CodWorkerPlace) ON DELETE SET NULL ON UPDATE CASCADE,
  CONSTRAINT FK_historymoved_workerplace_CodWorkerPlace2 FOREIGN KEY (CodNewPlace)
    REFERENCES workerplace(CodWorkerPlace) ON DELETE SET NULL ON UPDATE CASCADE
)
ENGINE = INNODB
AUTO_INCREMENT = 89
