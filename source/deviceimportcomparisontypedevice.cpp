...
DeviceImportComparisonTypeDevice::DeviceImportComparisonTypeDevice(QWidget *parent, const QString &reportFile, const QMap<QString,QVariant> comparisonTypeDev) :
    QDialog(parent),
    comparison(comparisonTypeDev)
{
    setupUi(this);
    ...
    if(comparison.isEmpty()){
        DeviceReportParser handler(false,true);
        ...
        for(int i = 0; i < handler.attrValues().count(); i++){
            ...
            populateCBox("CodTypeDevice","typedevice","",tr("<Выберите тип устройства>"),w2);
            connect(w2,SIGNAL(currentIndexChanged(int)),this,SLOT(setComparisonTypeDev(int)));
...
void DeviceImportComparisonTypeDevice::changeEvent(QEvent *e)
{
...
void DeviceImportComparisonTypeDevice::populateCBox(const QString &idName, const QString &tableName,
...
void DeviceImportComparisonTypeDevice::setComparisonTypeDev(int index)
{
...
void DeviceImportComparisonTypeDevice::on_buttonApplay_clicked()
{
