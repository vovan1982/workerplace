...
DeviceImportComparisonProducer::DeviceImportComparisonProducer(QWidget *parent, const QString &reportFile, const QMap<QString, QVariant> comparisonProducer) :
    QDialog(parent),
    comparison(comparisonProducer)
{
    setupUi(this);
    ...
    if(comparison.isEmpty()){
        DeviceReportParser handler(false,false,false,true);
        ...
        for(int i = 0; i < handler.attrValues().count(); i++){
            ...
            populateCBox("CodProducer","producer","","<Выберите производителя>",w2);
            connect(w2,SIGNAL(currentIndexChanged(int)),this,SLOT(setComparisonProducer(int)));
...
void DeviceImportComparisonProducer::changeEvent(QEvent *e)
{
...
void DeviceImportComparisonProducer::populateCBox(const QString &idName, const QString &tableName,
...
void DeviceImportComparisonProducer::setComparisonProducer(int index)
{
...
void DeviceImportComparisonProducer::on_buttonApplay_clicked()
{
